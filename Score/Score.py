    ...
    def insert(self, Aid, match_rate, intent, IPNO, PNO):
        ...
        if type(result) is str:
            if match_rate != empty:
                if intent != empty:
                    if IPNO != empty:
                        if PNO != empty:
                            Query().setExecute6(
                                "insert into answer (Aid, match_rate, intent, IPNO, PNO) values (%s, %s, %s, %s, %s)", Aid, match_rate, intent, IPNO, PNO)
            Aid = result
        else:
            ...
            if type(Luisid) is not str:
                if Luisid == Aid:
                    Query().setExecute6("update answer set match_rate=%s, intent=%s, IPNO=%s, PNO=%s where Aid = %s", match_rate, intent, IPNO, PNO, Aid)
            Aid = result
        return Aid
